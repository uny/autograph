...
- **Autocapture carries the scope, but attributes it by push order.** Autocaptured taps fire from
  ...
  and so do carry the scope and the screen. (Not the section: `TrackedScreen` takes no section, so
  nothing in the Compose API mirrors one — an autocaptured tap only carries a section when a native
  push site supplies it. `trackClick` / `trackImpression` still read section from
  `LocalScreenContext` as before.) That stack is ordered by when a scope was *mounted*,
