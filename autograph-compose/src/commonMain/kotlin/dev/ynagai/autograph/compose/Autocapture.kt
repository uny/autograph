 ...
 * on iOS relies on `testTag` alone — with one narrow exception outside this library's control:
 * Compose Multiplatform's own UIKit bridge falls back to an inline `LinkAnnotation.Clickable`'s
 * developer-supplied `tag` when no `testTag` is set (never to a `LinkAnnotation.Url`'s destination,
 * which resolves to no identifier at all), so such a link's `tag` can surface as the identifier too;
 * only elements exposing `UIAccessibilityTraitButton` are treated as clickable). Taps are silently
 * not captured on other targets (JVM/desktop).
 */
