 ...
 * behind. A/B on one process: a real Compose tap confirmed via `Element Clicked` in the same process,
 * immediately followed by a tap on an unrelated SwiftUI element, still resolved to nothing — identical
 * to a process no client had ever touched. So this pipeline's cold-inertness is not narrowed to
 * "pure-native apps with no Compose anywhere"; it applies to every hybrid app too, for any screen this
 * pipeline covers that a genuine accessibility client hasn't visited yet.
 *
